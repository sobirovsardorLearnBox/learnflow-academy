...
const notificationTypes = [
  ...
  { value: 'new_lesson', label: 'Yangi dars', icon: MessageSquare, color: 'bg-cyan-500' },
  { value: 'reminder', label: 'Eslatma', icon: AlertCircle, color: 'bg-orange-500' },
  { value: 'achievement', label: 'Yutuq', icon: CheckCircle2, color: 'bg-yellow-500' },
];
...
export default function TeacherNotifications() {
  ...
  // Fetch students from teacher's groups
  const { data: students, isLoading: studentsLoading } = useQuery({
    queryKey: ['teacher-students', user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      
      // Get all groups belonging to this teacher
      const { data: teacherGroups } = await supabase
        .from('groups')
        .select('id')
        .eq('teacher_id', user.id);
      
      if (!teacherGroups?.length) return [];
      
      const groupIds = teacherGroups.map(g => g.id);
      
      // Get approved members of these groups
      ...
      // Deduplicate students (may be in multiple groups)
      const uniqueStudents = new Map();
      members?.forEach(m => {
        if (!uniqueStudents.has(m.user_id)) {
          uniqueStudents.set(m.user_id, {
            user_id: m.user_id,
            name: (m.profiles as any)?.name || 'Noma\'lum',
  ...
  const validateForm = () => {
    ...
    if (targetType === 'student' && !selectedUserId) {
      setErrors({ target: 'Talabani tanlang' });
      return false;
    }
  ...
  const handleSend = async () => {
    ...
    try {
      const result = await sendNotification.mutateAsync({
        targetUserId: targetType === 'student' ? selectedUserId : undefined,
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Guruh talabalariga xabar yuborish</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card className="lg:col-span-2">
            <CardHeader>
              ...
              <CardDescription>
                O'z guruhlaringizdagi talabalarga xabar yuboring
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Tabs value={targetType} onValueChange={(v) => setTargetType(v as 'student' | 'group')}>
